...
export function AssignmentsPage() {
  ...
  const [submissionStats, setSubmissionStats] = useState<Map<string, SubmissionStats>>(new Map());
  ...
  const filteredAssignments = useMemo(() => {
    ...
    if (columnFilters.class.length > 0) {
      filtered = filtered.filter(a => columnFilters.class.includes(a.class?.name || ''));
    }

    if (columnFilters.subject.length > 0) {
      filtered = filtered.filter(a => columnFilters.subject.includes(a.subject?.name || ''));
    }

    if (columnFilters.teacher.length > 0) {
      filtered = filtered.filter(a => columnFilters.teacher.includes(a.teacher?.full_name || ''));
    }
  ...
  const getClassPerformanceData = useMemo(() => {
    ...
    const classGrouped = new Map<string, { totalScore: number; totalSubmissions: number; totalPossible: number; count: number }>();
    ...
    assignmentsForClass.forEach(assignment => {
      ...
      if (!classGrouped.has(className)) {
        classGrouped.set(className, { totalScore: 0, totalSubmissions: 0, totalPossible: 0, count: 0 });
      }
  ...
  const getSubmissionTrendData = useMemo(() => {
    ...
    return weeks.map((week, index) => {
      ...
      const avgSubmissionRate = totalStats.length > 0
        ? totalStats.reduce((sum, stat) => sum + stat.submissionPercentage, 0) / totalStats.length
        : 80;
  ...
  const getOverallStats = useMemo(() => {
    ...
    assignmentsForClass.forEach(assignment => {
      ...
      if (stats) {
        ...
        if (stats.submissionPercentage >= 80 && !isOverdue(assignment.due_date)) {
          onTimeCount += stats.submittedCount;
        }
      }
    });

    const avgCompletion = totalPossible > 0 ? Math.round((totalSubmissions / totalPossible) * 100) : 0;
    const onTimeSubmissions = totalSubmissions > 0 ? Math.round((onTimeCount / totalSubmissions) * 100) : 0;
    const avgScore = 85;
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl p-5 border-l-4 border-green-500 shadow-sm">
            ...
            <div className="text-xs text-green-600 dark:text-green-400 mt-1">Submission rate</div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl p-5 border-l-4 border-orange-500 shadow-sm">
            ...
            <div className="text-xs text-green-600 dark:text-green-400 mt-1">Timely submissions</div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl p-5 border-l-4 border-purple-500 shadow-sm">
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">Overall performance</div>
          </div>
        </div>
