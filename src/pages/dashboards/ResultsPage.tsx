...
import {
  TrendingUp, TrendingDown, Search, Download, Target,
  UserSearch, Users
} from 'lucide-react';
import {
...
export function ResultsPage() {
  ...
  const fetchStudentResults = async (searchTerm: string) => {
    ...
    try {
      const { data: profileData, error: profileError } = await supabase
        ...
        .or(`full_name.ilike.%${searchTerm}%,admission_no.ilike.%${searchTerm}%`)
        .eq('role', 'student')
        .limit(1)
        .maybeSingle();
  ...
  const calculateClassStats = (results: ExamResult[]) => {
    ...
    const finalStats = Array.from(statsMap.values()).map(stats => ({
      ...
      pass_percentage: Math.round((stats.pass_count / stats.total_students) * 100)
    }));
  ...
  const filteredResults = results.filter(result => {
    const matchesClass = selectedClasses.length === 0 || selectedClasses.includes(result.class_name);
    const matchesSubject = selectedSubjects.length === 0 || selectedSubjects.includes(result.subject_name);
    const matchesType = selectedTypes.length === 0 || selectedTypes.includes(result.exam_type);
    const matchesGrade = selectedGrades.length === 0 || selectedGrades.includes(result.grade);
  ...
  const getGradeBadge = (grade: string) => {
    const gradeConfig: Record<string, { bg: string; text: string }> = {
      ...
      'B': { bg: 'bg-blue-100 dark:bg-blue-900/30', text: 'text-blue-700 dark:text-blue-300' },
      'C': { bg: 'bg-yellow-100 dark:bg-yellow-900/30', text: 'text-yellow-700 dark:text-yellow-300' },
      'D': { bg: 'bg-orange-100 dark:bg-orange-900/30', text: 'text-orange-700 dark:text-orange-300' },
      'F': { bg: 'bg-red-100 dark:bg-red-900/30', text: 'text-red-700 dark:text-red-300' }
    };
  ...
  const studentSubjectPerformance = studentStats
    ? studentStats.subjects.map(subject => {
        const subjectResults = results.filter(r => r.subject_name === subject);
        const avg = subjectResults.reduce((sum, r) => sum + r.percentage, 0) / subjectResults.length;
        return {
          subject,
  ...
  const studentTrendData = results.slice(0, 10).reverse().map(r => ({
    exam: r.exam_code,
    marks: r.percentage
  }));
          ...
          {studentStats && searchMode === 'student' && (
            <>
              <div className="bg-gradient-to-r from-blue-500 to-blue-600 dark:from-blue-700 dark:to-blue-800 rounded-2xl p-6 text-white">
                <h3 className="text-xl font-bold mb-3">Student Performance Overview</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-blue-100 dark:text-blue-200 text-sm">Name</p>
                    <p className="text-lg font-semibold">{studentStats.student_name}</p>
                  </div>
                  <div>
                    <p className="text-blue-100 dark:text-blue-200 text-sm">Admission No</p>
                    <p className="text-lg font-semibold">{studentStats.admission_no}</p>
                  </div>
                  <div>
                    <p className="text-blue-100 dark:text-blue-200 text-sm">Class</p>
                    <p className="text-lg font-semibold">{studentStats.class_name}</p>
                  </div>
                  <div>
                    <p className="text-blue-100 dark:text-blue-200 text-sm">Total Exams</p>
                    <p className="text-lg font-semibold">{studentStats.total_exams}</p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="h-[300px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={studentTrendData}>
                        ...
                        />
                        <Line type="monotone" dataKey="marks" stroke="#3B82F6" strokeWidth={3} name="Marks %" />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                    ...
                    <tr className="border-b-2 border-gray-200 dark:border-gray-700">
                      ...
                      <th className="text-left py-3 px-4 font-semibold text-gray-700 dark:text-gray-300">
                        <div className="flex items-center gap-1">
                          ...
                          <ColumnFilter
                            column="Class"
                            values={results.map(r => r.class_name)}
                      ...
                      <th className="text-left py-3 px-4 font-semibold text-gray-700 dark:text-gray-300">
                        <div className="flex items-center gap-1">
                          ...
                          <ColumnFilter
                            column="Subject"
                            values={results.map(r => r.subject_name)}
                      ...
                      <th className="text-left py-3 px-4 font-semibold text-gray-700 dark:text-gray-300">
                        <div className="flex items-center gap-1">
                          ...
                          <ColumnFilter
                            column="Type"
                            values={results.map(r => r.exam_type)}
                  ...
                  <tbody>
                    {filteredResults.map((result) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-4 text-gray-600 dark:text-gray-400">{result.class_name}</td>
                        <td className="py-3 px-4 text-gray-600 dark:text-gray-400">{result.subject_name}</td>
                        <td className="py-3 px-4 text-gray-600 dark:text-gray-400 capitalize">{result.exam_type}</td>
                        ...
                        <td className="py-3 px-4 text-center">{getGradeBadge(result.grade)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
