...
export function ExamsPage() {
  ...
  const [footfallData, setFootfallData] = useState<Map<string, ExamFootfall>>(
    new Map()
  );
  ...
  const fetchFootfallData = async (examIds: string[]) => {
    try {
      ...
      examIds.forEach((examId) => {
        const examAttendance = (data || []).filter(
          (a: any) => a.exam_id === examId
        );
        const total = examAttendance.length;
        const present = examAttendance.filter(
          (a: any) => a.status === 'present'
        ).length;
        const absent = examAttendance.filter(
          (a: any) => a.status === 'absent'
        ).length;
        const late = examAttendance.filter(
          (a: any) => a.status === 'late'
        ).length;
  ...
  const filteredExams = exams.filter((exam) => {
    ...
    const matchesClass =
      ...
      selectedClasses.includes(exam.class_name || '');
    const matchesSubject =
      selectedSubjects.length === 0 ||
      selectedSubjects.includes(exam.subject_name || '');
    const matchesType =
      selectedTypes.length === 0 || selectedTypes.includes(exam.exam_type);
    const matchesStatus =
      selectedStatuses.length === 0 || selectedStatuses.includes(exam.status);
    ...
    try {
      const { error } = await supabase.from('exams').insert({
        ...
        passing_marks: formData.passing_marks
          ? parseInt(formData.passing_marks)
          : 0,
  ...
  const handleOpenEditExam = (exam: Exam) => {
    ...
    setEditFormData({
      ...
      end_time: exam.end_time || '',
      duration_minutes: exam.duration_minutes
        ? exam.duration_minutes.toString()
        : '',
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          total_marks: editFormData.total_marks
            ...
            : 0,
          passing_marks: editFormData.passing_marks
            ? parseInt(editFormData.passing_marks)
            : 0,
                    ...
                    {filteredExams.map((exam) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-3 px-3 sm:px-4 text-gray-600 dark:text-gray-400 capitalize">
                            {exam.exam_type}
                          </td>
                          <td className="py-3 px-3 sm:px-4 text-gray-600 dark:text-gray-400">
                          ...
                          <td className="py-3 px-3 sm:px-4 text-center">
                            <div className="flex items-center justify-center gap-1">
                              ...
                              <span className="text-gray-600 dark:text-gray-400">
                                {footfall?.present_count || 0}/
                                {footfall?.total_students || 0}
                              </span>
                            </div>
                          </td>
                          <td className="py-3 px-3 sm:px-4 text-center">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full">
            ...
            >
              ...
              <div className="bg-gray-50 dark:bg-gray-900/40 rounded-lg p-3 mb-4 text-sm">
                ...
                <p className="text-gray-500 dark:text-gray-400">
                  {examToDelete.class_name || 'N/A'} •{' '}
                  {examToDelete.subject_name || 'N/A'}
                </p>
              </div>
